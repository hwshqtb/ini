        ...
        namespace impl {
            ...
            HWSHQTB__INLINE parse_status parse_fixed_size_integer(std::string_view sv, T& v, std::uint8_t size) {
                ...
                while (size--) {
                    ...
                    if (is_space(c)) break;
                    T digit = is_xdigit(c);
                    if (digit == -1 || digit >= 15) return {sv, false};
                    v = v * 10 + digit;
                }
                return {sv, true};
            }
...
namespace hwshqtb {
    namespace ini {
                    ...
                    if (c == '\\' && quote == '\"') {
                        ...
                        switch (c) {
                            ...
                            case 'u': {
                                ...
                                if (const auto& [nsv, succeed] = impl::parse_fixed_size_integer(sv, codepoint, 4); sv = nsv, !succeed) return {sv, false};
                            ...
                            case 'U': {
                                ...
                                std::uint64_t codepoint;
                                if (const auto& [nsv, succeed] = impl::parse_fixed_size_integer(sv, codepoint, 8); sv = nsv, !succeed) return {sv, false};
        ...
        parse_status parse(std::string_view sv, integer& v) {
            v = 0;
            if (sv.empty()) return {sv, false};
            char* end = const_cast<char*>(sv.data()) + sv.size();
            v = strtoll(sv.data(), &end, 10);
            if (end == sv.data() || (end != sv.data() + sv.size() && std::localeconv()->decimal_point[0] == *end)) return {sv, false};
        ...
        parse_status parse(std::string_view sv, floating& v) {
            ...
            if (sv.empty()) return {sv, false};
            char* end = const_cast<char*>(sv.data()) + sv.size();
            v = strtod(sv.data(), &end);
            if (end == sv.data()) return {sv, false};
            sv.remove_prefix(end - sv.data());
            return {sv, true};
        }
